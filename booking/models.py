...
class Table(models.Model):
    """
    Model to store the tables along with it capacity
    Need to be added by an admin using the admin panel
    The tables are not intented to be changed so if need any change
    needs to do it manually using the admin panel.
    """
...
class BookingTime(models.Model):
    """
    Model to store data and the ralation between Booking and table
    ...
    def __str__(self):
        return (f"Booking: {self.booking} | Table: {self.table} | "
