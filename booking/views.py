...
def FindBooking(request):
    """
    This view checks if there its a booking available with the data provided
    by the user.
    ...
    if request.method == 'POST':
        ...
        available_tables = find_available_tables(
            aware_booking_datetime, guests, int(booking_id)
            )
        if available_tables:
            ...
            return JsonResponse(
                {'available': False, 'alternatives': alternatives}
                )
...
def make_booking(request):
    ...
    If the user needs to book more than a table create multiple
    BookingTime instances with the same Booking id
    """
    if request.method == "POST":
        # Get the data
...
def find_available_tables(booking_datetime, guests, booking_id):
    ...
    for table in all_tables:
        ...
        # the start_time its greater than/equal to the date-BOOKING_DURATION,
        # and the end_time its lesser than/equal to the date+BOOKING_DURATION
    ...
    for table in available_tables:
        ...
        if capacity_count >= guests:
            # If there are enough tables availables to fit the number of guest
            # then return booking_tables
...
def find_alternatives(booking_datetime, guests, booking_id):
    ...
    while len(alternatives) < 3:
        available_tables = []
        capacity_count = 0
        # Iterate tables to check if there any available table at that time
        for table in Table.objects.all():
            if not BookingTime.objects.exclude(
                ...
                available_tables.append(table)
                capacity_count += table.capacity
                # If finds enough tables to reach the number of guests
                # appends the table ids and the time
                if capacity_count >= guests:
                    alternatives.append({
                        'table_id': '-'.join(
                            str(table.table_id) for table in available_tables
                            ),
...
def my_bookings(request):
    ...
    active_bookings = Booking.objects.filter(
        user=request.user,
        date__gt=current_time).order_by('date')
    # Get the past bookings of the user
    past_bookings = Booking.objects.filter(
        user=request.user,
        date__lte=current_time).order_by('-date')
...
def modify_booking(request, booking_id):
    ...
    if booking.user == request.user:
        ...
        messages.add_message(
            request, messages.ERROR, 'You can only modify your own bookings!')
    return redirect('my_bookings')
...
def get_data(request):
    ...
    if button in [
        ...
        if button == 'make-booking':
            ...
            time_key = f"{button}-time"
            date = request.POST.get(time_key).rstrip('Z')

        # Make aware the datetime
        if button == 'make-booking':
            date_format = '%Y-%m-%d %H:%M'
            booking_datetime = make_aware(datetime.strptime(date, date_format))
        else:
        ...
        if button == 'make-booking':
            ...
            table_id_key = f"{button}-table_id"

        table_id = request.POST.get(table_id_key)
